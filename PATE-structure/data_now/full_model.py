...
# from skal-models.__init__ import *
...
from data_now.alexnet import alexnet
from data_now.googlenet import googlenet
from data_now.resnet import resnet18, resnet34, resnet50, resnet101
from data_now.vgg import vgg16_bn


# from models import googlenet


# from models import resnet18, resnet34, resnet50, resnet101, alexnet, vgg16_bn, googlenet
...
class FullModel(nn.Module):
    ...
    def featmap_norm(self, feat_map):
        ...
        feat_map = F.interpolate(feat_map, size=(25, 25), mode='bilinear', align_corners=True).squeeze(dim=1)
    ...
    def img_sampling(self, img, h_str, h_end, w_str, w_end):
        ...
        # img_sampled = torch.zeros(img_b, img_c, int(img_h / 2), int(img_w / 2))
        ...
        for i in range(img_b):
            ...
            img_sampled[i, :] = F.interpolate(img_sampled_i, size=(int(img_h / 2), int(img_w / 2)), mode='bilinear', align_corners=True)
    ...
    def save(self, net, path):
        ...
        torch.save(net, f'{path}/model.pt')
    ...
    def confidences_to_classes(confidences: np.ndarray) -> np.ndarray:
        _, classes = torch.nn.functional.softmax(confidences, dim=-1).topk(1)
